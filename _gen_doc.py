...
def find_all_files(pkg_path, result):
    ...
    if file starts with '.' or '_', this function would ignore it and it would not be included
    in `result`
    """
...
class TableContent:
    ...
    def write(self):
        with open('./doc/table.md', 'w') as file:
            write_content = '\n'.join(self.content)
...
class DocumentContent:
    ...
    def write_document(cls, module_name, module_dict, path):
        ...
        if not os.path.isdir('./doc/documents'):
            os.makedirs('./doc/documents')
            
        with open(f'./doc/documents/{module_name}.md', 'w') as file:
            file.write(write_content)
            

def gen_table_document(root_path, include=None, ignore=None):
    if include is None:
        include = []
    if ignore is None:
        ignore = []
    ...
    for ff in pls:
        ...
        if os.path.isdir(ff):
            ...
            table.update_content(module_name, module_dict)
...
if __name__ == '__main__':
    include = []
    ignore = ['./hurricane_tools/config.py']
    gen_table_document('./hurricane_tools/', include, ignore)